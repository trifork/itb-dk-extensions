...
| `compose.https.yml` | Adds pinned Caddy, persistent certificates, public HTTPS ports and URLs, forwarded-header support for CDA, and loopback-only HTTP bootstrap ports. | `start.sh` when `ITB_HTTPS_HOST` is set |
| `Caddyfile` | Routes HTTPS to the UI gateway on 9000, callbacks on 8080, and the validator on 8080 via public port 8443; automatically obtains and renews certificates, sets one-year HSTS on both HTTPS endpoints and preserves application CSP headers. | `compose.https.yml` |
